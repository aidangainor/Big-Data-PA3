...
public class LemmaVectorizer {
	...
	public LemmaVectorizer(String dictPath) 
			throws IOException {
		this.wordsAndIndexMap = createVectorMappingForAllLemmas(dictPath);
	}
	 ...
	 * originalLemmaPath: a string path to a list of all unique lemmas in original dict
	 * This can be generated by the GetAllLemmas mapred call
	 */
	public HashMap<String, Integer> createVectorMappingForAllLemmas(String dictPath) 
			throws IOException {
		int uniqueWordCount = 0;
		FileSystem fs = FileSystem.get(new Configuration());
		HashMap<String, Integer> wordsAndIndexMap = new HashMap<String, Integer>();
		System.out.println("lemma path = " + dictPath);
		Path pt = new Path(dictPath);
		...
		while ((line = originalLemmaReader.readLine()) != null) {
			...
			if (!wordsAndIndexMap.containsKey(trimLine)) {
				wordsAndIndexMap.put(trimLine, uniqueWordCount);
				uniqueWordCount++;
			}
		}
